...
void Graphics::fillTriangle(Vertex& vMin, Vertex& vMid, Vertex& vMax)
{
	// 1. Sort the Vertices w.r.t. 'y'
	if (vMid.m_vPosition.y < vMin.m_vPosition.y)	std::swap(vMin, vMid);
	if (vMax.m_vPosition.y < vMid.m_vPosition.y)	std::swap(vMid, vMax);
	if (vMid.m_vPosition.y < vMin.m_vPosition.y)	std::swap(vMin, vMid);

	Edge eTopToBottom;			eTopToBottom.set(vMin.m_vPosition, vMax.m_vPosition);
	Edge eTopToMiddle;			eTopToMiddle.set(vMin.m_vPosition, vMid.m_vPosition);
	Edge eMiddleToBottom;		eMiddleToBottom.set(vMid.m_vPosition, vMax.m_vPosition);
	{
		bool bHandedness = isRightHandedTriangle(&vMin.m_vPosition, &vMax.m_vPosition, &vMid.m_vPosition);
...
void Graphics::scanEdges(Edge* eA, Edge* eB, bool bHandedness)
{
	...
	int yStart = eB->m_pVertexPositionStart.y;
	int yEnd = eB->m_pVertexPositionEnd.y;
	for (int y = yStart; y <= yEnd; y++)						// Increment the value of 'y' by 1 pixel at a time.
	{
		float iLeftX = eLeft->m_pVertexPositionVarying.x;		// Varying is stepped for each 'y' increment
		float iRightX = eRight->m_pVertexPositionVarying.x;		// 'x' has the value associated for each 'y' step.

		for (int32_t x = iLeftX; x <= iRightX; x++)
		{
			setPixel(x, y, m_RenderColour);
		}
