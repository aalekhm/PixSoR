...
void Graphics::fillTriangleInternal(VertexInternal& vMinVert, VertexInternal& vMidVert, VertexInternal& vMaxVert, bool bIsInScreenSpace)
{
	if (!bIsInScreenSpace)
	{
		...
		TRANSFORM_HOMOGENEOUS_TO_SCREENCOORDINATES(vMinVert);
		TRANSFORM_HOMOGENEOUS_TO_SCREENCOORDINATES(vMidVert);
		TRANSFORM_HOMOGENEOUS_TO_SCREENCOORDINATES(vMaxVert);
	}
