...
class MyEngine : public GameEngine
{
	public:
		MyEngine()
		{
			createWindow(480, 320, "15_Camera");
		}
		...
		virtual void onCreate()
		{
			...
			OpenGL::m_pCamera = std::make_unique<Camera>(gMatProjection);
#if THE_2_TRIANGLES
			Vertex pVertices[6];
			{
			...
			gTerrain->setShader( std::make_unique<VertexTextureShader>() );
			{
				IShader* pShader = gTerrain->getShader();
				((VertexTextureShader*)pShader)->m_pUniform_WVPMatrix = &gTerrain->getTransformation();
			}
#endif
		}
