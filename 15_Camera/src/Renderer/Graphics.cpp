...
m_pDepthBuffer = (float*)std::malloc(m_iMaxBufferSize * sizeof(float)); \
m_pClearDepthBuffer = (float*)std::malloc(m_iMaxBufferSize * sizeof(float)); \
{ \
...
void Graphics::clear()
{
	...
	memcpy(m_pDepthBuffer, m_pClearDepthBuffer, m_iMaxBufferSize * sizeof(float));
}
...
void Graphics::setClearColourAndDepth(Pixel pPixel, float fClearDepth)
{
	...
	for (int32_t i = 0; i < m_iMaxBufferSize; i++)
	{
		...
		m_pClearDepthBuffer[i] = fClearDepth;
	}
}
...
void Graphics::scanEdges(Edge* eA, Edge* eB, bool bHandedness)
{
	...
	for (int y = yStart; y <= yEnd; y++)						// Increment the value of 'y' by 1 pixel at a time.
	{
		...
		{
			int32_t iPos = x + y * m_iWidth;

			if (iPos >= m_iMaxBufferSize) // Discard the Pixel if it lies outside the 'Buffer Area'.
				continue;

			// Depth Test
			float fDepthAtPixel = LERP(GET_EDGE_VARYING_INTERNAL(eLeft, EVaryingsInternal::DEPTH_VARYING), GET_EDGE_VARYING_INTERNAL(eRight, EVaryingsInternal::DEPTH_VARYING), fXLerpAmount);
			if (fDepthAtPixel < m_pDepthBuffer[iPos])
			{
				m_pDepthBuffer[iPos] = fDepthAtPixel;
				funcRenderPixel(x, y);
			}
		}
