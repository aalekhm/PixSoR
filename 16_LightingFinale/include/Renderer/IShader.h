...
class VertexTextureShader : public IShader
{
	public:
		COMPILE(ATTRIBUTE_LIST((&m_pAttribute_Position, &m_pAttribute_TexCoord)), VARYINGS_LIST((&m_pVarying_TexCoord)))
		...
		virtual void		vertex()
		{
			...
			// TEX COORD
			{
				SET(m_pVarying_TexCoord) = GET(m_pAttribute_TexCoord);
			}
		}

		virtual bool		fragment()
		{
			gl_Colour = OpenGL::sample2D(m_pVarying_TexCoord->x, m_pVarying_TexCoord->y);
			return true;
		}

		ATTRIBUTE(Vector4f, m_pAttribute_Position)
		ATTRIBUTE(Vector4f, m_pAttribute_TexCoord)

		UNIFORM(Matrix4f, m_pUniform_WVPMatrix)

		VARYING(Vector4f, m_pVarying_TexCoord)
	protected:
	private:
};

class VertexLightShader : public IShader
{
	public:
		COMPILE(ATTRIBUTE_LIST((&m_pAttribute_Position, &m_pAttribute_TexCoord, &m_pAttribute_Normal)), VARYINGS_LIST((&m_pVarying_TexCoordU, &m_pVarying_TexCoordV, &m_pVarying_LightIntensity)))
		...
		virtual void		vertex()
		{
			...
			// TEX COORD
			{
				SET(m_pVarying_TexCoordU) = m_pAttribute_TexCoord->x;
				SET(m_pVarying_TexCoordV) = m_pAttribute_TexCoord->y;
			}

			// NORMAL
			{
				Vector4f vNormal = GET(m_pAttribute_Normal);

				float fDotProduct = vNormal.dot(m_pUniform_Light->m_vDirection);
				CLAMP(0.0f, fDotProduct, 1.0f);
				float fLightIntensity = (fDotProduct * m_pUniform_Light->m_fDirectionIntensity) + m_pUniform_Light->m_fAmbientIntensity;
				
				SET(m_pVarying_LightIntensity) = fLightIntensity;
			}
		}

		virtual bool		fragment()
		{
			Pixel pP = OpenGL::sample2D(GET(m_pVarying_TexCoordU), GET(m_pVarying_TexCoordV));

			gl_Colour.m_iRed	= (uint32_t)((pP.m_iRed & 0xFF) * GET(m_pVarying_LightIntensity));
			gl_Colour.m_iGreen	= (uint32_t)((pP.m_iGreen & 0xFF) * GET(m_pVarying_LightIntensity));
			gl_Colour.m_iBlue	= (uint32_t)((pP.m_iBlue & 0xFF) * GET(m_pVarying_LightIntensity));
			gl_Colour.m_iAlpha	= 0xFF;
		...
		ATTRIBUTE(Vector4f, m_pAttribute_Position)
		ATTRIBUTE(Vector4f, m_pAttribute_TexCoord)
		ATTRIBUTE(Vector4f, m_pAttribute_Normal)

		UNIFORM(Matrix4f,	m_pUniform_WVPMatrix)
		UNIFORM(Light,		m_pUniform_Light)

		VARYING(float, m_pVarying_TexCoordU)
		VARYING(float, m_pVarying_TexCoordV)
		VARYING(float, m_pVarying_LightIntensity)
	protected:
	private:
};