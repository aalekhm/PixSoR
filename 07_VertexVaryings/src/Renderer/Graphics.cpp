...
void Graphics::fillTriangleInternal(VertexInternal& vMinVert, VertexInternal& vMidVert, VertexInternal& vMaxVert)
{
...
void Graphics::scanEdges(Edge* eA, Edge* eB, bool bHandedness)
{
	...
	for (int y = yStart; y <= yEnd; y++)						// Increment the value of 'y' by 1 pixel at a time.
	{
		...
		RENDER_PIXEL_CALLBACK
		{
			...
			{
				Pixel pRenderPixel(255, 255, 255, 255);
				{
					Vector4f vColour = *(Vector4f*)(m_pVaryingsLerpedValues + 4); // +4 cause, position is the 1st Vector4f
					pRenderPixel = vColour * 255;
					pRenderPixel.m_iAlpha = 255;
				}

				m_pFrameBuffer->setPixel(x, y, pRenderPixel);
			}
